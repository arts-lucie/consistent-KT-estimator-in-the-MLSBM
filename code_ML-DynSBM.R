...
pen_ML <- function(k, n,T, eps = 10^-8) {
  if (k == 1) { return(0) }
  x <- 1:(k - 1)
  pen <- ((T*x*(x+1) + (x -1))/2 + 1 + eps)  * log(n)
  return(sum(pen))
}

pen_dyn <- function(k, n,T, eps = 10^-8) {
  if (k == 1) { return(0) }
  x <- 1:(k - 1)
  pen <- x/2 * log(T*n^2)+ x*(x-1)/2 *log(n*T) + (T*x*(x-1)/2 +1+ eps)*log(n)
  return(sum(pen))
}
...
k_chap_dyn <- function(ml_G, qmax=10, nbiter=10,fpnbiter=10,emeps=10^-8,fpeps=10^-8,n0=0.5, eta0=0.5, zeta0=0.5){
  n <- vcount(ml_G[[1]])
  m <- matrice_aretes(ml_G)
  res <- VariationalBayes(N=n,m = m, qmin =1, qmax=min(qmax,n), nbiter=nbiter, fpnbiter=fpnbiter, emeps=emeps, fpeps=fpeps, directed=FALSE, n0=n0, eta0=eta0, zeta0=zeta0)
  log_KT <- sapply(1:min(qmax,n), function(l) res[[l]]$KT)
  pen <- sapply(1:min(qmax,n), function(l) pen_dyn(l,n,T))
  test <- log_KT-pen
  k <- which.max(test)
  return(k)
}
